...
rain.lim <- median(eventEx$rainfall, na.rm = T)
...
eventEx <- eventEx %>%                                                                        # pipe
  ...
  mutate(rainfall.flag =                                                                      # Generate rainfall timestamp flag
           ...
           ifelse( is.na(rainfall),NA,                                                        # if rainfall is NA flag is "NA"    
                   ...
                   ifelse( rainfall <= rain.lim,"",                                           # elseif if rainfall is <= the rain limit flag is blank
                           
                           ifelse( (lag(rainfall) <= rain.lim | is.na(lag(rainfall))) 
                                   & (lead(rainfall) <= rain.lim | is.na(lead(rainfall))),
                                   "ISOLATE",                                                 # elseif lag and lead rainfall is <= the rain limit or NA flag is ISOLATE
                                  
                                    ifelse((lag(rainfall) > rain.lim & !is.na(lag(rainfall))) 
                                          & (lead(rainfall) > rain.lim & !is.na(lead(rainfall))),
                                          "LINK",                                             # elseif lag and lead rainfall is > the rain limit and not NA flag is LINK
                                          
                                          ifelse(lag(rainfall) <= rain.lim | is.na(lag(rainfall)), 
                                                 "START",                                     # elseif preceeding rainfall is <= the rain limit or NA flag is START
