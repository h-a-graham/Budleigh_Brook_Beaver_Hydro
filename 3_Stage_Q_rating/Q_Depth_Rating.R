...
Flow_path1 <- file.path(here(), "1_Clean_Flow_Data/exports/EB_Flow_round1.rds")
Flow_path2 <- file.path(here(),"1_Clean_Flow_Data/exports/EB_Flow_round2.rds")
EB_Stage_path <- file.path(here(),"2_Join_Clean_Stage_Data/exports/EB_Stage_Clean.rds")
...
# get extents etc for plot
noNa <- stage.df[!is.na(stage.df$stage),]
nrow(noNa[noNa$stage >0.8,])
stage.df %>%
  summarise(maxim=max(stage, na.rm = T),
            minim =min(stage, na.rm = T))
...
rating_wide_CP <- ggplot(Q.S.select, aes(x=stage, y=flow_ls)) +
  ...
  geom_vline(xintercept = depth.zeroQ, linetype = 2) + # zero flow line.
  annotate(geom = "text", x = depth.zeroQ - 0.01 , y = 5000, label = "depth at zero-flow", angle = 90) +
  scale_x_continuous(limits = c(0.4,0.88),breaks = seq(0.4,0.88, by = 0.05)) +
  scale_y_continuous(limits = c(0,20000), breaks = seq(0,20000, by = 5000)) +
