...
# ------------- Read Data ------------------------------
hyd_dat <- read_rds("./5_event_extraction/eventExtraction__beaver/Pophams/run_20200710_1222_value__padding2880_alpha0.98_passes3_BFI0.74/eventEx_EVENTS_metrics.rds")

all_flow <- read_rds('4_Join_Rain_to_Q/exports/Pophams_Q_R_S_ts.rds') %>% # reload flow inout for calculating correct Excedence...
  select(q) %>%
  drop_na()
...
hyd_dat2 <- hyd_dat %>%
  ...
  # select_if(~ !any(is.na(.))) %>% #perhaps should be filter?
...
ggplot(hyd_dat2, aes(x = rain.tot.mm, y = Q.peak.m3.s, colour = Beaver, fill = Beaver)) +
  ...
  stat_smooth(method = "glm", se = TRUE,
              method.args = list(family = Gamma(link='identity')), linetype = 1) +
  stat_smooth(method = "glm", se = TRUE, geom = "ribbon",
              method.args = list(family = Gamma(link='identity')), alpha = 0.2,linetype = 2) +
...
# glm with Gamma fit - still lots of issues with residuals and no convergance...

# Both of these models converge but give very different results!

m2 <- glm(Q.peak.m3.s ~ Beaver + rain.tot.mm, data= hyd_dat2, family = Gamma(link='identity'), start = c(0.05,0, 0.01))

m2b <- glm(Q.peak.m3.s ~ Beaver * rain.tot.mm, data= hyd_dat2, family = Gamma(link='identity'), start = c(0.02,0.02, 0.02, 0))
...
# glm with Hydrological Seasona as additional variable...Still issues...

m3 <- glm(Q.peak.m3.s ~ Hydro.Seas + rain.mean * Beaver ,data= hyd_dat2, family = Gamma(link='identity'))
summary(m3)

check_model(m3)

# m3 %>% broom.mixed::augment() %>%
...
m3 %>%
  augment(type.predict = "response",
          type.residuals = "deviance",
          se_fit = T) %>%
  ggplot(., aes(x=rain.mean, y=.fitted, colour=Beaver))+
  geom_point(aes(y=Q.peak.m3.s))+
  geom_line() +
  coord_cartesian(ylim=c(0,10))+
...
gam1d <- gam(Q.peak.m3.s ~ s(rain.mean, by=Beaver, bs='cs') + Beaver, 
...
ggplot(hyd_dat2, aes(x = rain.mean, y = Q.peak.m3.s, colour = Beaver, fill = Beaver)) +
...
ggplot(hyd_dat3, aes(x = rain.tot.mm, y = per_q, colour = Beaver, fill = Beaver)) +
  geom_point(alpha = 0.5) +
...
m5 <- glm(per_q ~ rain.tot.mm + Beaver, data= hyd_dat3, family = Gamma(link='log'))
...
new_dat.m5 <- Create_Data(.data=hyd_dat3, var='rain.tot.mm')
...
# Main Plot
Exc.glm.plt <- ggplot(m5_df, aes(x = rain.tot.mm, y = .fitted, colour = Beaver, fill = Beaver)) +
...
m6 <- glm(per_q ~ rain.tot.mm + Beaver + Hydro.Seas, data= hyd_dat3, family = Gamma('log')) # I think this is the appropriate option here...
...
new_dat.m6 <- Create_Data(.data=hyd_dat3, var='rain.tot.mm')
...
Exc.glm.plt.hs <- ggplot(m6_df, aes(x = rain.tot.mm, y = .fitted, colour = Beaver, fill = Beaver)) +
  geom_point(data=hyd_dat3, aes(y=per_q), alpha = 0.5, size=0.8) +
  geom_ribbon(aes(ymin = .fitted - (1.96 *.se.fit), ymax = .fitted + (1.96 *.se.fit)), alpha=0.2, linetype=2, lwd=0.2) +
  geom_line() +
  scale_color_manual(values = c('#A6190D', '#244ED3')) +
...
qr.mod <- rq(Q.peak.m3.s ~ rain.tot.mm + Beaver, tau = c(0.25, 0.5, 0.7, 0.9), data= hyd_dat2)
...
rq.new_dat <- Create_Data(.data=hyd_dat2, var='rain.tot.mm')

rq_pred_func <- function(.data, new.data, q){
  qr.mod <- rq(Q.peak.m3.s ~ rain.tot.mm + Beaver, tau = c(q), data= .data)
...
QuRe.plot <- ggplot(qr.se.df, aes(x = rain.tot.mm, y = fit, colour = Beaver, fill = Beaver)) +
...
# basic ANCOVA - Diagnostics show massive heteroscedaticity and lack of normallity... Reject
m7 <- lm(Q.peak.m3.s ~ rain.mean + Beaver, data= big_storms)
summary(m7)
...
m8  <- glm(Q.peak.m3.s ~ rain.tot.mm + Beaver, data= big_storms, family = Gamma(link='identity'))
...
m9  <- glm(Q.peak.m3.s ~ rain.tot.mm + Beaver, data= Wet_Ante_df, family = Gamma(link='identity'))
...
new_datc <- Create_Data(.data=Wet_Ante_df, var='rain.tot.mm')
...
wet.anti.plt <- ggplot(Wet_Ante_df, aes(x=rain.tot.mm, y = Q.peak.m3.s, colour = Beaver, fill = Beaver)) +
  geom_point(alpha = 0.5)+
...
full_flow <- read_rds('4_Join_Rain_to_Q/exports/Pophams_Q_R_S_ts.rds') %>%
